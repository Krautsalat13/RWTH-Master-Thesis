...
RelabelIndicesInTerm[expr_,unique_:False, relabelExtLeg_:False]:=Block[
	{
		ind,
		indexCounter,
		newInd,
	...
	If[unique,
		...
		If[
			...
			,
			dummyIndices = FindDummyIndices[expr /. Field[EXT[_],___]:>1];
		];
	...
	,
		...
		If[
			...
			,
			{dummyIndices, openInds} = FindIndices[expr /. Field[EXT[_],___]:>1];
		];
...
Options[RelabelIndices] = {Expand->True, Unique->False, ExtLeg->False};
...
RelabelIndices[expression:Except[_Plus], OptionsPattern[]]:=Block[

	{
		...
		relabelExtLeg = OptionValue[ExtLeg]
	},
	...
	Plus@@ (RelabelIndicesInTerm[#, unique, relabelExtLeg]&/@ expr)
]
...
Options[ContractDelta] = {Expand->True, ExtLeg->False};


(* Performance improvements for long expressions *)
ContractDelta[expr_Plus, OptionsPattern[]] := ContractDelta[#, Expand->OptionValue[Expand], ExtLeg->OptionValue[ExtLeg]]&/@expr
...
ContractDelta[arg:Except[_Plus], OptionsPattern[]] :=
	Module[
		...
		If[Head[expr]===Plus,
			expr = List@@expr;
			Plus@@ (ContractDeltaSingleTerm[#, Expand->OptionValue[Expand], ExtLeg->OptionValue[ExtLeg]]&/@ expr)
		,
			ContractDeltaSingleTerm[expr, Expand->OptionValue[Expand], ExtLeg->OptionValue[ExtLeg]]
		]
	]


Options[ContractDeltaSingleTerm] = {Expand->True, "overall-only"->False, ExtLeg->False};
...
ContractDeltaSingleTerm[expr_, OptionsPattern[]] :=
	Module[
		...
		If[
			!OptionValue[ExtLeg]
			,
			noDelta = noDelta /. Field[EXT[_],___]:>1
		];
...
ContractMetricSingleTerm@ term_Times:= Block[{res, mets},
	...
	(* Inactivate Index for external legs for Feynman rules *)
	res = res /. f : Field[EXT[args__], rest___] :> Inactivate[f, Index];
